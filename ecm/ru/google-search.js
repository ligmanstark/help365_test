...
if (/elma365.com\/ru\/help/i.test(currentLocalHref) || /elma365.com\/en\/help/i.test(currentLocalHref) || /http:\/\/127.0.0.1/i.test(currentLocalHref) || /http:\/\/localhost/i.test(currentLocalHref)) {
   ...
   `
    ...
    document.addEventListener('DOMContentLoaded', () => {
        //не показывать контент пока плашка не загрузится
        // document.body.classList.add('hidden');

        function outputSubCategoryResult(url) {
            switch (true) {
                case (/127.0.0.1/i.test(url)):
                    return 'localhost';
                case (/localhost/i.test(url)):
                    return 'localhost';
                case (/elma365.com\/ru\/help\/platform/i.test(url)):
                    return 'Платформа';
                case (/elma365.com\/ru\/help\/ecm/i.test(url)):
                    return 'ECM';
                case (/elma365.com\/ru\/help\/crm/i.test(url)):
                    return 'CRM';
                case (/elma365.com\/ru\/help\/projects/i.test(url)):
                    return 'Проекты';
                case (/elma365.com\/ru\/help\/business_solutions/i.test(url)):
                    return 'Бизнес-решения';
                case (/elma365.com\/ru\/help\/service/i.test(url)):
                    return 'Сервис';
                default:
                    return '';
            }
        ...
        function insertCurrentSubDirectory() {
            ...
            const text = outputSubCategoryResult(href);
            if (!text) {
                // Если текст пустой, удаляем плашку если она есть
                // const old = document.getElementById('subcategory');
                // if (old) old.remove();
                document.body.classList.remove('hidden');
                return;
            }

            // Проверяем, существует ли уже плашка с таким же текстом
            const existing = document.getElementById('subcategory');
            ...
            subcategory.className = 'search-res__item-category search-res__item-category_subcategory subcategory';
            subcategory.textContent = text;

            requestAnimationFrame(() => {
                const breadcrumbs = document.querySelector('.topic__breadcrumbs');
                const articleTitle = document.querySelector('.article__header');

                // Проверяем, есть ли уже плашка с таким текстом
                const existing = document.querySelector('#subcategory');
                if (existing && existing.textContent === subcategory.textContent) {
                    console.log('Плашка уже существует, не вставляем повторно');
                    document.body.classList.remove('hidden');
                    return;
                }

                // Удалим другие плашки, если есть (например, дубли от мутаций)
                const duplicates = document.querySelectorAll('.subcategory');
                duplicates.forEach(el => {
                    if (el !== existing) el.remove();
                });

                if(!document.querySelectorAll('#subcategory')){
                    breadcrumbs.prepend(subcategory);
                }
                if (breadcrumbs) {
                    breadcrumbs.prepend(subcategory);
        ...
        // Отслеживание смены URL в SPA (переходы по истории и pushState)
        ...
        // На случай, если переход вручную по DOM
        ...
        // // Обработка события из первого фрагмента
        document.addEventListener('subcategory:update', () => {
            setTimeout(insertCurrentSubDirectory, 100);
        });
    });
    ////////////////////////////////////////////////////////////////////////
