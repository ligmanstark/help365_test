...
function searchFormHeader() {
  $('.search-form').each(function () {
    $(this).on('submit', (evt) => {
      ...
      if (searchStr.trim()) {
        if ((/help365_test\/(platform|ecm|crm|projects|business_solutions|service)\/(ru|en)/i.test(window.location.href) || /elma365.com\/ru\/help/i.test(window.location.href) || /elma365.com\/en\/help/i.test(window.location.href) || /t-elma365.com/i.test(window.location.href) || /([\d+]{3}).(\d).(\d).(\d)/i.test(window.location.href) || /http:\/\/localhost/i.test(window.location.href)) && !triggerTitleMegaGuide) {
