...
const addCanonicalUrl = () => {
  const urlMapping = {
    '/ru/help/business_solutions/platform-distribution.html': '/ru/help/platform/platform-distribution.html'
  };
...
// const queryAnchor = window.location.hash;
 //  const normalizedHash = decodeURIComponent(queryAnchor.slice(1));
 //  let anchorElement = null;
 //  if (normalizedHash) {
 //    anchorElement = document.querySelector(`[name="${normalizedHash}"]`);
 //    if (!anchorElement) {
 //      anchorElement = document.getElementById(normalizedHash);
 //    }
 //    const headingElement = anchorElement.nextSibling;
 //
 //    if (headingElement) {
 //      // 1. Запрещаем показ хедера
 //      preventHeaderShow = true;
 //
 //      // 2. Плавный скролл к элементу
 //      headingElement.scrollIntoView({
 //        behavior: 'smooth',
 //        block: 'start'
 //      });
 //
 //      // 3. Разрешаем показ хедера через 1 секунду (после завершения скролла)
 //      setTimeout(() => {
 //        preventHeaderShow = false;
 //      }, 2000);
 //    }}
//ТЕСТ
const queryAnchor = window.location.hash;
...
//
//
//     if (preventHeaderShow && st < lastScrollTop) {
//       lastScrollTop = st;
//       return;
//     }
//
...
//     history.replaceState(null, null, window.location.pathname); // убираем #anchor из адресной строки
//     window.scrollTo(0, 0); // скроллим в начало
//   }
// });
//
      ...
      item.addEventListener('click', evt => {
        ...
        if (url !== window.location.href) {
          if (!url.includes('javascript:void')) {
           ...
           //      if (nestedReal) {
           //        nestedReal.click(); //  кликаем по вложенной реальной ссылке
           //      }
           //    }
           //
...
if (/elma365.com\/ru\/help/i.test(currentUrl) || /elma365.com\/en\/help/i.test(currentUrl) || /elma365\/help\/ru/i.test(currentUrl) || /elma365\/help\/en/i.test(currentUrl) || /([\d+]{3}).(\d).(\d).(\d):([\d+]{4})\/index.html/i.test(currentUrl) || /([\d+]{3}).(\d).(\d).(\d)\/index.html/i.test(currentUrl)) {
  window.addEventListener('load', () => {
...
function searchFormHeader() {
  $('.search-form').each(function () {
    $(this).on('submit', (evt) => {
      ...
      if (searchStr.trim()) {
        if ((/elma365.com\/ru\/help/i.test(window.location.href) || /elma365.com\/en\/help/i.test(window.location.href) || /t-elma365.com/i.test(window.location.href) || /([\d+]{3}).(\d).(\d).(\d)/i.test(window.location.href) || /http:\/\/localhost/i.test(window.location.href)) && !triggerTitleMegaGuide) {
